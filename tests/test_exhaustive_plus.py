...
def test_periodic_arena_wrapping():
    ...
    wrapped_x = wrap_coordinates(
        np.array([config.grid_x_max + 10]), config.grid_x_min, config.grid_x_max
    )
    wrapped_y = wrap_coordinates(
        np.array([config.grid_y_min - 10]), config.grid_y_min, config.grid_y_max
    )
...
def test_adaptation_perception():
    ...
    op = LeakAdaptationPerception(
        odor_integration_tau=0.1, adaptation_tau=1.0, adaptation_magnitude=0.5
    )
    op.reset()
...
def test_constant_odor_release_kernel():
    ...
    instructions = odor_release.release_odor(
        1, (0, 0), 0, config, np.random.default_rng()
    )
...
def test_default_odor_release():
    ...
    instructions = odor_release.release_odor(
        0, (0, 0), 0, SimulationConfig(), np.random.default_rng()
    )
    assert instructions == []
...
def test_default_behavior_heading():
    behavior = DefaultBehavior()
    config = SimulationConfig(T=1, fps=1)
    new_heading = behavior.update_heading(
        0, 5, 10, False, config, np.random.default_rng(42)
    )
...
def test_runner_parallel_serial():
    ...
    behavior = DefaultBehavior()
    odor_release = DefaultOdorRelease()
    serial_results = run_simulations(
        config, behavior, arena, odor_release, n_replicates=2, parallel=False
    )
    parallel_results = run_simulations(
        config, behavior, arena, odor_release, n_replicates=2, parallel=True
    )
...
def test_visualization_plots(tmp_path):
    ...
    viz.plot_trajectories_with_odor(
        sim_index=0, show=False, save_path=str(out_dir / "traj.png"), wraparound=False
    )
    viz.plot_final_odor_grid(
        sim_index=0, show=False, save_path=str(out_dir / "odor.png")
    )
    viz.plot_odor_time_series(
        sim_index=0, show=False, save_path=str(out_dir / "odor_ts.png")
    )
