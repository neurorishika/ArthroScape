...
def simulate_replicate_vectorized(config: SimulationConfig, behavior: BehaviorAlgorithm, arena: Arena,
                                  odor_release_strategy: OdorReleaseStrategy, seed: int = None) -> Dict[str, Any]:
    simulator = MultiAnimalSimulator(config, behavior, arena, odor_release_strategy, seed)
...
def run_simulations_vectorized(config: SimulationConfig, behavior: BehaviorAlgorithm, arena: Arena,
                                 odor_release_strategy: OdorReleaseStrategy, n_replicates: int = 1,
                                 parallel: bool = True) -> List[Dict[str, Any]]:
     results = []
     seeds = [np.random.SeedSequence().entropy for _ in range(n_replicates)]
     if parallel:
          with ProcessPoolExecutor() as executor:
                futures = {executor.submit(simulate_replicate_vectorized, config, behavior, arena, odor_release_strategy, seed): i for i, seed in enumerate(seeds)}
                for future in tqdm(as_completed(futures), total=n_replicates, desc="Simulating"):
                 results.append(future.result())
     else:
          for seed in tqdm(seeds, desc="Simulating"):
                results.append(simulate_replicate_vectorized(config, behavior, arena, odor_release_strategy, seed))
     return results